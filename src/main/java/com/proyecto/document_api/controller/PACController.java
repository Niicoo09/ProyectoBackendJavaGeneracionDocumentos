...
public class PACController {
    ...
    public ResponseEntity<byte[]> generateDeclaracionPropietario(@PathVariable UUID id) {
        return processPacResponse(id, "pac/DeclaracionPropietario", "2.- Declaración del Propietario", "declaracion-propietario", formData -> {
    ...
    public ResponseEntity<byte[]> generateAutorizacionFacturacion(@PathVariable UUID id) {
        return processPacResponse(id, "pac/AutorizacionFacturacion", "1.- Autorización de Facturación", "autorizacion-facturacion", formData -> {
            Map<String, String> extraImages = new HashMap<>();
